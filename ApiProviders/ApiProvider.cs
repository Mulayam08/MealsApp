...
namespace MealApp.ApiProviders
{
    public class ApiProvider : IApiProvider
    {
        ...
        public ApiResult<T> GetFile<T>(string url, Dictionary<string, string> headers = null)
        {
            HttpResponseMessage result = null;
            try
            {
                ...
                var rawStream = result.Content.ReadAsStreamAsync().Result;
                ...
                if (rawStream != null)
                {
                    ...
                    {
                        int read;
                        while ((read = rawStream.Read(imageArray, 0, imageArray.Length)) > 0)
                        {
                ...
                try
                {
                    return new ApiResult<T>(rawStream, (int)result.StatusCode, imageArray);
                }
                catch (Exception ex)
                {
                    return new ApiResult<T>(rawStream, 501, null);
                }
            }
            catch (Exception e)
            {
